...
function updatePricingSummary() {
    ...
    if (orderDetails) {
        orderDetails.innerHTML = `
            <p><strong>Cloud<i>Test</i> Go - 3 Day Trial</strong> × <strong>${deviceCount}</strong> devices @ <strong>$${devicePrice}/device</strong> = <strong>$${deviceTotal.toLocaleString()}</strong></p>
            ${addonDescription}
        `;
    }
...
function loadFinalSummary() {
    ...
    itemsHTML += `
        <div class="item-row">
            <div class="item-info">
                <div class="item-name">Cloud Test Go - 3 Day Trial</div>
...
function populateFoxyCartFields() {
    ...
    const mainProductName = 'Cloud Test Go - 3 Day Trial (per device)';
    const productCode = `cloudtestgo-trial-3day-${deviceCount}dev`;
    ...
    if (selectedAddons.length > 0) {
        ...
        const addonProductName = `Add-ons: ${selectedAddonNamesForProduct.join(', ')} - 3 Day Trial (per device)`;
        const addonCode = `addons-${selectedAddons.join('-')}-trial-3day`;
