// ========================================
// CloudTest Lite - Trial
// ========================================

// Coupon Validation URL
const COUPON_CHECK_URL = 'https://ctlite.yasir-07a.workers.dev/';
...
function updatePricingSummary() {
    ...
    if (orderDetails) {
        orderDetails.innerHTML = `
            <p><strong>Cloud<i>Test</i> Lite - 1 Month Trial</strong> - <strong>${deviceCount}</strong> devices × <strong>$${devicePrice}</strong> = <strong>$ ${deviceTotal.toLocaleString()}</strong></p>
            ${addonDescription}
        `;
    }
...
function loadFinalSummary() {
    ...
    itemsHTML += `
        <div class="item-row">
            <div class="item-info">
                <div class="item-name">Cloud Test Lite - 1 Month Trial</div>
...
function populateFoxyCartFields() {
    ...
    const mainProductName = 'CloudTest Lite - 1 Month Trial (per device)';
    const productCode = `cloudtestlite-trial-monthly-${deviceCount}dev`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('CloudTest Lite form initialized successfully');
});