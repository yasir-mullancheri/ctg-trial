...
const COUPON_CHECK_URL = 'https://trial3and7.yasir-07a.workers.dev/';
...
const PRICING = {
    ...
    bundleDiscount: 50  // discount when all 3 addons selected
};
...
document.getElementById('countrySearch')?.addEventListener('input', function(e) {
    ...
    document.querySelectorAll('#countryGrid .selection-card').forEach(card => {
        ...
        card.style.display = countryName.includes(searchTerm) ? 'flex' : 'none';
    });
});
...
function handleSelectAllFilters(checked) {
    ...
    if (currentStep === 5) populateDevices();
}
...
/**
 * Calculate addon total based on new pricing:
 * - Automation+:  $25/device
 * - Experience+:  $75/device
 * - Performance+: $75/device
 * - All 3 together: sum - $50 bundle discount
 * - 2 addons: no discount, just sum of individual prices
 */
function calculateAddonTotal(deviceCount) {
    if (selectedAddons.length === 0) return 0;

    // Sum individual prices
    let sumPerDevice = selectedAddons.reduce((total, addon) => {
        return total + PRICING.addons[addon];
    }, 0);

    // Apply $50/device discount only when all 3 addons selected
    if (selectedAddons.length === 3) {
        sumPerDevice -= PRICING.bundleDiscount;
    }
...
function updatePricingSummary() {
    ...
    const devicePrice = PRICING.devicePrice;
    ...
    if (selectedAddons.length > 0) {
        const selectedAddonNames = selectedAddons.map(a => addonNames[a]);
        const sumPerDevice = selectedAddons.reduce((t, a) => t + PRICING.addons[a], 0);

        let discountNote = '';
        if (selectedAddons.length === 3) {
            discountNote = ` <em>($${PRICING.bundleDiscount} bundle discount applied per device)</em>`;
            const discountedPerDevice = sumPerDevice - PRICING.bundleDiscount;
            addonDescription = `<p><strong>${selectedAddonNames.join(', ')}</strong> × <strong>${deviceCount}</strong> devices @ <strong>$${discountedPerDevice}/device</strong> = <strong>$${addonTotal.toLocaleString()}</strong>${discountNote}</p>`;
        } else {
            addonDescription = `<p><strong>${selectedAddonNames.join(', ')}</strong> × <strong>${deviceCount}</strong> devices = <strong>$${addonTotal.toLocaleString()}</strong></p>`;
        }
    }
    ...
    if (orderDetails) {
        orderDetails.innerHTML = `
            <p><strong>Cloud<i>Test</i> Go - 7 Days Trial</strong> × <strong>${deviceCount}</strong> devices @ <strong>$${devicePrice}/device</strong> = <strong>$${deviceTotal.toLocaleString()}</strong></p>
            ${addonDescription}
        `;
    }
...
// ========================================
// Coupon Validation Functions (Step 6)
// ========================================
...
function hideCouponMessage() {
    const couponMessage = document.getElementById('couponMessage');
    if (couponMessage) couponMessage.classList.remove("show");
}

// ========================================
// Coupon Validation (Step 7 Receipt)
// ========================================
...
function loadFinalSummary() {
    ...
    itemsHTML += `
        <div class="item-row">
            <div class="item-info">
                <div class="item-name">Cloud Test Go - 7 Days Trial</div>
    ...
    if (selectedAddons.length > 0) {
        ...
        let detailStr = `${deviceCount} devices`;
        if (selectedAddons.length === 3) {
            const discountedPerDevice = sumPerDevice - PRICING.bundleDiscount;
            detailStr += ` × $${discountedPerDevice}/device (bundle discount applied)`;
        }
        ...
        itemsHTML += `
            <div class="item-row">
                <div class="item-info">
                    ...
                    <div class="item-details">${detailStr}</div>
                </div>
...
function populateFoxyCartFields() {
    ...
    const deviceCount = Object.keys(selectedDevices).length;
    const devicePrice = PRICING.devicePrice;
    const billingCycle = '7 Days Free Trial';

    // Calculate addon price per device for FoxyCart line item
    let addonPricePerDevice = 0;
    if (selectedAddons.length > 0) {
        const sumPerDevice = selectedAddons.reduce((t, a) => t + PRICING.addons[a], 0);
        addonPricePerDevice = selectedAddons.length === 3
            ? sumPerDevice - PRICING.bundleDiscount
            : sumPerDevice;
    }

    const mainProductName = 'Cloud Test Go - 7 Days Trial (per device)';
    const productCode = `cloudtestgo-trial-7day-${deviceCount}dev`;
    ...
    // Add-on as second product (if any)
    if (selectedAddons.length > 0) {
        const selectedAddonNamesForProduct = selectedAddons.map(a => addonNames[a]);
        const addonProductName = `Add-ons: ${selectedAddonNamesForProduct.join(', ')} - 7 Days Trial (per device)`;
        const addonCode = `addons-${selectedAddons.join('-')}-trial-7day`;
